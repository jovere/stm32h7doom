...
static void gfx_draw_img_rgb565 (gfx_image_t* img, gfx_coord_t* coord)
{
	...
	hdma2d.Init.OutputOffset = GFX_MAX_WIDTH - coord->source_w;
	...
	if (HAL_DMA2D_PollForTransfer(&hdma2d, 1000) != HAL_OK) {
		fatal_error("DMA2D transfer failed");
	}
}
...
static void gfx_draw_img_argb8888 (gfx_image_t* img, gfx_coord_t* coord)
{
	...
	// Configure DMA2D handle for blending mode
	hdma2d.Instance = DMA2D;
	...
	// Configure foreground layer (source image)
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].InputOffset = picture_width - coord->source_w;
	...
	if (HAL_DMA2D_ConfigLayer(&hdma2d, DMA2D_FOREGROUND_LAYER) != HAL_OK) {
		fatal_error("DMA2D foreground config failed");
	}

	// Configure background layer (destination)
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].InputOffset = GFX_MAX_WIDTH - coord->source_w;
	...
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].RedBlueSwap = DMA2D_RB_REGULAR;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].AlphaInverted = DMA2D_REGULAR_ALPHA;

	if (HAL_DMA2D_ConfigLayer(&hdma2d, DMA2D_BACKGROUND_LAYER) != HAL_OK) {
		fatal_error("DMA2D background config failed");
	}
...
{
	// Configure DMA2D handle for Register-to-Memory mode (clear screen)
	hdma2d.Instance = DMA2D;
	...
	// Extract color components and create fill color
	uint8_t r = GFX_RGB565_R(color);
	uint8_t g = GFX_RGB565_G(color);
	uint8_t b = GFX_RGB565_B(color);
	uint32_t fill_color = (0xFF << 24) | (r << 16) | (g << 8) | b;

	// Start fill operation for entire screen
	if (HAL_DMA2D_Start(&hdma2d, fill_color, lcd_frame_buffer, GFX_MAX_WIDTH, GFX_MAX_HEIGHT) != HAL_OK) {
		fatal_error("DMA2D clear screen start failed");
	}
...
void gfx_fill_rect (gfx_image_t* img, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t color)
{
	...
	// Configure DMA2D handle for Register-to-Memory mode (fill)
	hdma2d.Instance = DMA2D;
	hdma2d.Init.Mode = DMA2D_R2M;
	hdma2d.Init.ColorMode = gfx_convert_colormode(img->pixel_format);
	hdma2d.Init.OutputOffset = img->width - width;
	hdma2d.Init.AlphaInverted = DMA2D_REGULAR_ALPHA;
	hdma2d.Init.RedBlueSwap = DMA2D_RB_REGULAR;
	...
	uint32_t fill_color = (a << 24) | (r << 16) | (g << 8) | b;
	if (HAL_DMA2D_Start(&hdma2d, fill_color, address, width, height) != HAL_OK) {
		fatal_error("DMA2D fill start failed");
	}
...
void gfx_draw_img_on_img (gfx_image_t* img_src, gfx_image_t* img_dst, gfx_coord_t* coord)
{
	...
	uint16_t width_src;
	uint16_t width_dst;
	uint32_t src_color_mode, dst_color_mode;
	...
	// Convert output color modes to input color modes
	src_color_mode = (img_src->pixel_format == GFX_PIXEL_FORMAT_RGB565) ? DMA2D_INPUT_RGB565 : DMA2D_INPUT_ARGB8888;
	dst_color_mode = (img_dst->pixel_format == GFX_PIXEL_FORMAT_RGB565) ? DMA2D_INPUT_RGB565 : DMA2D_INPUT_ARGB8888;

	// Configure DMA2D handle for blending mode
	hdma2d.Instance = DMA2D;
	hdma2d.Init.Mode = DMA2D_M2M_BLEND;
	hdma2d.Init.ColorMode = gfx_convert_colormode(img_dst->pixel_format);
	hdma2d.Init.OutputOffset = width_dst - coord->source_w;
	hdma2d.Init.AlphaInverted = DMA2D_REGULAR_ALPHA;
	hdma2d.Init.RedBlueSwap = DMA2D_RB_REGULAR;
	...
	// Configure foreground layer (source image)
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].InputOffset = width_src - coord->source_w;
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].InputColorMode = src_color_mode;
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].AlphaMode = DMA2D_NO_MODIF_ALPHA;
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].InputAlpha = 0xFF;
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].RedBlueSwap = DMA2D_RB_REGULAR;
	hdma2d.LayerCfg[DMA2D_FOREGROUND_LAYER].AlphaInverted = DMA2D_REGULAR_ALPHA;
	...
	// Configure background layer (destination image)
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].InputOffset = width_dst - coord->source_w;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].InputColorMode = dst_color_mode;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].AlphaMode = DMA2D_NO_MODIF_ALPHA;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].InputAlpha = 0xFF;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].RedBlueSwap = DMA2D_RB_REGULAR;
	hdma2d.LayerCfg[DMA2D_BACKGROUND_LAYER].AlphaInverted = DMA2D_REGULAR_ALPHA;
	...
	// Start blending transfer
	if (HAL_DMA2D_BlendingStart(&hdma2d, address_src, address_dst, address_dst, coord->source_w, coord->source_h) != HAL_OK) {
		fatal_error("DMA2D blending start failed");
	}
